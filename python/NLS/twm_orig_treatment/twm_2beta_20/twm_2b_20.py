#Calculates three-wave-mixing effects at the 2*beta frequency for beta=20.
import math as mth
...
for k in range(nbmax):
    ...
    for j in range(nmaxz):
        ...
        chi_2b=2*beta*t-4*(beta**2)*z
        s1=s1+mth.sin(2*chib-chi0)/((mth.cosh(xb)**2))
        s2=s2+mth.cos(2*chib-chi0)/((mth.cosh(xb)**2))
        z=z+dz
    phi[k]=2.0*(etab**2)*etanb*dz*mth.sqrt(s1**2+s2**2)/mth.cosh(x0)
    if abs(t)<delta:
