...
def update_plot(AA_dict):
	global pointSet, bondLines

	# Update graphical lattice
	pointSet.set_xdata(zip(*AA_dict.keys())[0])
	pointSet.set_ydata(zip(*AA_dict.keys())[1])
...
while step < Nsteps:
	...
	Energy.append(energy(AA_dict_new) )
	step += 1

	if AA_dict == AA_dict_new: continue # no need to update if plot is same
...
pp.plot(TS,Energy)
pp.show()
#pp.savefig('lattice_protein_2D.png')
